...
describe('GoYoshi', () => {
  ...
  describe('buildReleasePullRequest', () => {
    it('filters out submodule commits', async () => {
      ...
      expect(pullRequestBody).toEqual(expect.not.arrayContaining(['logging']));
  ...
  describe('getIgnoredSubModules', () => {
    ...
    it('fetches the list of submodules', async () => {
      ...
      const ignoredSubModules = await strategy.getIgnoredSubModules();
      expect(ignoredSubModules.size).toEqual(2);
      expect(ignoredSubModules.has('storage')).toBe(true);
      expect(ignoredSubModules.has('pubsub')).toBe(true);
    });
  });
});
