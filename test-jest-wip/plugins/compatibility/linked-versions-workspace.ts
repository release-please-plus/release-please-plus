...
describe('Plugin compatibility', () => {
  ...
  describe('linked-versions and workspace', () => {
    it('should version bump dependencies together', async () => {
      ...
      expect(updater.version.toString()).toEqual('1.1.0');
      const updaterB = (
        assertHasUpdates(
          pullRequest.updates,
      ...
      expect(updaterB.version.toString()).toEqual('1.1.0');
      const manifestUpdate = assertHasUpdate(
        pullRequest.updates,
        '.release-please-manifest.json',
        CompositeUpdater
      );
      expect(manifestUpdate.updater).toBeInstanceOf(CompositeUpdater);
      const manifestUpdaters = (manifestUpdate.updater as CompositeUpdater)
        .updaters;
      expect(manifestUpdaters).not.toBeEmpty();
      for (const manifestUpdater of manifestUpdaters) {
        expect(manifestUpdater).toBeInstanceOf(ReleasePleaseManifest);
        const versionsMap = (manifestUpdater as ReleasePleaseManifest)
          .versionsMap;
        expect(versionsMap).toBeDefined();
