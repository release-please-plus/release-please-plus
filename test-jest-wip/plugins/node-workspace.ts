...
function assertHasVersionUpdate(update: Update, expectedVersion: string) {
  expect(update.updater).toBeInstanceOf(RawContent);
  const updater = update.updater as RawContent;
  const data = JSON.parse(updater.rawContent);
  expect(data.version).toEqual(expectedVersion);
}
...
describe('NodeWorkspace plugin', () => {
  ...
  describe('run', () => {
    ...
    it('handles a single node package', async () => {
      ...
      expect(nodeCandidate).toBeDefined();
    ...
    it('walks dependency tree and updates previously untouched packages', async () => {
      ...
      expect(updater.versionsMap?.get('node2')?.toString()).toEqual('2.2.3');
      expect(updater.versionsMap?.get('node3')?.toString()).toEqual('1.1.2');
      expect(updater.versionsMap?.get('node5')?.toString()).toEqual('1.0.1');
      expect(
        dateSafe(nodeCandidate!.pullRequest.body.toString())
      ).toMatchSnapshot();
    });
