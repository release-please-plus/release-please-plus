...
describe('PullRequestTitle', () => {
  describe('parse', () => {
    describe('autorelease branch name', () => {
      it('parses a versioned branch name', () => {
        ...
        expect(pullRequestTitle).toBeDefined();
        expect(pullRequestTitle?.getTargetBranch()).toBeUndefined();
        expect(pullRequestTitle?.getComponent()).toBeUndefined();
        expect(pullRequestTitle?.getVersion()?.toString()).toEqual('1.2.3');
        expect(pullRequestTitle?.toString()).toEqual(name);
      });
      ...
      it('parses a versioned branch name with component', () => {
        ...
        expect(pullRequestTitle).toBeDefined();
        expect(pullRequestTitle?.getTargetBranch()).toBeUndefined();
        expect(pullRequestTitle?.getComponent()).toEqual('storage');
        expect(pullRequestTitle?.getVersion()?.toString()).toEqual('1.2.3');
      });
    });
    ...
    it('parses a target branch', () => {
      ...
      const pullRequestTitle = PullRequestTitle.parse(name);
      expect(pullRequestTitle).toBeDefined();
      expect(pullRequestTitle?.getTargetBranch()).toEqual('main');
      expect(pullRequestTitle?.getComponent()).toBeUndefined();
      expect(pullRequestTitle?.getVersion()?.toString()).toEqual('1.2.3');
    });
    ...
    it('parses a target branch and component with a slash', () => {
      ...
      expect(pullRequestTitle).toBeDefined();
      expect(pullRequestTitle?.getTargetBranch()).toEqual('main');
      expect(pullRequestTitle?.getComponent()).toEqual('some/title-test');
      expect(pullRequestTitle?.getVersion()?.toString()).toEqual('0.0.1');
    });

    it('fails to parse', () => {
      const pullRequestTitle = PullRequestTitle.parse('release-foo');
      expect(pullRequestTitle).toBeUndefined();
    });
  });
  describe('ofVersion', () => {
    it('builds the autorelease versioned branch name', () => {
      ...
      expect(pullRequestTitle.toString()).toEqual('chore: release 1.2.3');
    });
  });
  describe('ofComponentVersion', () => {
    it('builds the autorelease versioned branch name with component', () => {
      ...
      expect(pullRequestTitle.toString()).toEqual(
  ...
  describe('ofTargetBranch', () => {
    it('builds branchname with only target branch', () => {
      ...
      expect(pullRequestTitle.toString()).toEqual('chore(main): release');
    });
  });
  describe('ofTargetBranchVersion', () => {
    it('builds branchname with target branch and version', () => {
      ...
      expect(pullRequestTitle.toString()).toEqual('chore(main): release 1.2.3');
    });
  });
...
describe('PullRequestTitle with custom pullRequestTitlePattern', () => {
  describe('parse', () => {
    ...
    it('parses a component with @ sign prefix', () => {
      ...
      expect(pullRequestTitle).toBeDefined();
      expect(pullRequestTitle?.getComponent()).toEqual('@example/storage');
    });
    ...
    it('parses a manifest title', () => {
      ...
      expect(pullRequestTitle?.getTargetBranch()).toEqual('main');
      expect(pullRequestTitle?.getComponent()).toBeUndefined();
      expect(pullRequestTitle?.getVersion()).toBeUndefined();
    });
    ...
    it('parses a complex title and pattern', () => {
      ...
      expect(pullRequestTitle).toBeDefined();
      expect(pullRequestTitle?.getTargetBranch()).toEqual('hotfix/v3.1.0-bug');
      expect(pullRequestTitle?.getVersion()?.toString()).toEqual(
        '3.1.0-hotfix1'
      );
      expect(pullRequestTitle?.getComponent()).toEqual('@example/storage');
    });
