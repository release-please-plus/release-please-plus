...
describe('VersionManifest', () => {
  describe('parseVersions', () => {
    it('parses multiple versions in versions.txt', async () => {
      ...
      expect(versionsMap.get('google-cloud-trace')?.toString()).toBe(
        '0.108.0-beta'
      );
      expect(versionsMap.get('grpc-google-cloud-trace-v1')?.toString()).toBe(
        '0.73.0'
      );
      expect(versionsMap.get('grpc-google-cloud-trace-v2')?.toString()).toBe(
        '0.73.0'
      );
      expect(versionsMap.get('proto-google-cloud-trace-v1')?.toString()).toBe(
        '0.73.0'
      );
      expect(versionsMap.get('grpc-google-cloud-trace-v2')?.toString()).toBe(
        '0.73.0'
  ...
  describe('needsSnapshot', () => {
    it('parses detects a release version', async () => {
      ...
      expect(VersionsManifest.needsSnapshot(content)).toBe(true);
    });
    ...
    it('parses detects an existing snapshot version', async () => {
      ...
      expect(VersionsManifest.needsSnapshot(content)).toBe(false);
    });
  });
