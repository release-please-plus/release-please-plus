...
describe('commit-exclude', () => {
  ...
  it('should not exclude anything if paths are empty', () => {
    ...
    expect(newCommitsPerPath['.'].length).toBe(3);
    expect(newCommitsPerPath['pkg1'].length).toBe(1);
    expect(newCommitsPerPath['pkg2'].length).toBe(1);
    expect(newCommitsPerPath['pkg3'].length).toBe(2);
  });
  ...
  it('should not exclude only if all files are from excluded path', () => {
    ...
    const commitExclude = new CommitExclude(config);
    const newCommitsPerPath = commitExclude.excludeCommits(commitsPerPath);
    expect(newCommitsPerPath['.'].length).toBe(2);
    expect(newCommitsPerPath['pkg1'].length).toBe(1);
    expect(newCommitsPerPath['pkg2'].length).toBe(1);
    expect(newCommitsPerPath['pkg3'].length).toBe(1);
  });
  ...
  it('should exclude if all files are from excluded path', () => {
    ...
    const commitExclude = new CommitExclude(config);
    const newCommitsPerPath = commitExclude.excludeCommits(commitsPerPath);
    expect(newCommitsPerPath['.'].length).toBe(1);
    expect(newCommitsPerPath['pkg1'].length).toBe(1);
    expect(newCommitsPerPath['pkg2'].length).toBe(1);
    expect(newCommitsPerPath['pkg3'].length).toBe(2);
  });
  ...
  it('should make decision only on relevant files', () => {
    ...
    expect(newCommitsPerPath['a'].length).toBe(1);
    expect(newCommitsPerPath['d'].length).toBe(0);
  });
});
