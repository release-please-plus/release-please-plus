...
describe('SentenceCase Plugin', () => {
  ...
  describe('processCommits', () => {
    it('converts description to sentence case', async () => {
      ...
      expect(commits[0].message).toBe('fix: Hello world');
      expect(commits[1].message).toBe('fix: Goodnight moon');
    });
    it('leaves reserved words lowercase', async () => {
      ...
      expect(commits[0].message).toBe('feat: gRPC can now handle proxies');
      expect(commits[1].message).toBe('fix: npm now rocks');
    });
    it('handles sentences with now breaks', async () => {
      ...
      expect(commits[0].message).toBe('feat: Beep-boop-hello');
      expect(commits[1].message).toBe('fix: Log4j.foo.bar');
    });
  });
  ...
  it('handles subject with multiple : characters', async () => {
    ...
    expect(commits[0].message).toBe('fix: Hello world:goodnight moon');
  });
